...
def main():
    ...
    config = EmbeddedTrainingConfig(
        ...
        # Dataset
        dataset_name="mrkswe/endpointDatasetFT",

        # Output settings
        output_dir="./embedded_outputs",
        hub_model_id="mrkswe/phi4-model02-endpoint",  # Your model name
        hf_token="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",

        # Embedded behavior - BU DAVRANIŞI GÖMER!
        task_prefix="Endpoint seçimi:",
        task_description=(
            "Senin görevin, kullanıcı mesajına en uygun endpoint'i seçmektir. Sadece bir tane endpoint adını yaz. Aksi bir durum olursa False yaz."
            "Kullanıcının mesajındaki amacı analiz et. Hangi endpoint bu amaca hizmet ediyorsa, sadece o endpoint adını döndür. Sadece endpointin adını ekrana yaz başka hiçbir şey yazma"
                          ),
